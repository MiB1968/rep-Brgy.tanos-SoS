 ...
 * NOTE: Do not use in web apps where tokens live in localStorage —
 * the fetch function already reads from localStorage automatically.
 */
...
export async function customFetch<T = unknown>(
  ...
  if (_authTokenGetter && !headers.has("authorization")) {
    ...
    // FIX: consolidated into single branch — removed redundant else-if
    const token = typeof window !== "undefined" ? localStorage.getItem("brgy_token") : null;
    if (token) {
      headers.set("authorization", `Bearer ${token}`);
    }
  }
  ...
  if (!response.ok) {
    ...
    // FIX: auto-logout when the server rejects our token (revoked, expired
