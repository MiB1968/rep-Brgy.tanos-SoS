...
export default function ResidentDashboard() {
  ...
  function doCreate(location: { lat: number; lng: number }, type = "OTHER", description = "SOS triggered by resident") {
    playSirenSound();
    createAlert.mutate(
      { data: { type: type as any, location, description } },
      {
        onSuccess: () => {
                ...
                <div className="space-y-2">
                  <div className="h-1.5 bg-[#040B1A] rounded-full overflow-hidden">
                    <motion.div
                      ...
                      animate={{ width: STATUS_BAR[activeAlert.status] || "33%" }}
                    />
                  </div>
        ...
        <div className="flex-1 w-full space-y-6">
          ...
          <motion.div variants={itemVariants} className="space-y-4">
            ...
            {Array.isArray(broadcasts) && broadcasts.length === 0 ? (
              ...
              <div className="space-y-2">
                {Array.isArray(broadcasts) && (broadcasts as Broadcast[]).slice(0, 3).map((b) => (
